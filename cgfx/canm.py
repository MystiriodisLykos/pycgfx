...
class AnimationCurve(StandardObject):
    ...
    def values(self) -> tuple:
        return (self.start_frame, self.end_frame,
                self.pre_repeat_method, self.post_repeat_method,
                self.flags)
...
class FloatSegment(StandardObject):
    ...
    def refresh_struct(self):
        ...
        if self.single_value is not None:
            ...
            struct += 'if'
            if self.quantization not in (QuantizationType.Hermite128, QuantizationType.UnifiedHermite96, QuantizationType.StepLinear64):
                struct += 'fff'
            for k in self.keys:
                k.refresh_struct()
    ...
    def values(self):
        return (self.start_frame, self.end_frame, ((self.single_value is not None) | (self.interpolation << 2) | (self.quantization << 5))
            ) + ((self.single_value,) if self.single_value is not None else (
                len(self.keys),
                # speed is used to accelerate lookups
                # for this, time * speed * (num_keys-1) must < num_keys
                # so speed < 1 / time
                1 / (self.end_frame - self.start_frame),
            ) + ((self.scale, self.offset, self.frame_scale) if self.quantization not in (QuantizationType.Hermite128, QuantizationType.UnifiedHermite96, QuantizationType.StepLinear64) else ()
            ) + tuple(self.keys))
...
class FloatAnimationCurve(AnimationCurve):
    ...
    def __init__(self) -> None:
        self.segments = []
    def refresh_struct(self):
        self.struct = Struct(AnimationCurve.struct.format + 'i' + 'i' * len(self.segments))
    def values(self) -> tuple:
...
class Vector3AnimationCurve(AnimationCurve):
    ...
    def __init__(self) -> None:
        self.frames = []
    def refresh_struct(self):
        self.struct = Struct(AnimationCurve.struct.format + Vector3AndFlags.format * len(self.frames))
    def values(self) -> tuple:
        if (self.flags[0] & 1) == 0:
...
class QuaternionAnimationCurve(AnimationCurve):
    ...
    def __init__(self) -> None:
        self.frames = []
    def refresh_struct(self):
        self.struct = Struct(AnimationCurve.struct.format + QuaternionAndFlags.format * len(self.frames))
    def values(self) -> tuple:
        if (self.flags[0] & 1) == 0:
            assert len(self.frames) == self.end_frame - self.start_frame
        return super.values() + (
...
class CANMBoneTransform(CANMBone):
    ...
    def refresh_struct(self):
        flags = TransformFlag(0)
        struct = ''
        values = (self.scale_x, self.scale_y, self.scale_z, self.rot_x, self.rot_y, self.rot_z, self.pos_x, self.pos_y, self.pos_z)
        ignore_flags = (TransformFlag.ScaleXIgnore, TransformFlag.ScaleYIgnore, TransformFlag.ScaleZIgnore, TransformFlag.RotXIgnore, TransformFlag.RotYIgnore, TransformFlag.RotZIgnore, TransformFlag.PosXIgnore, TransformFlag.PosYIgnore, TransformFlag.PosZIgnore)
        const_flags = (TransformFlag.ScaleXConst, TransformFlag.ScaleYConst, TransformFlag.ScaleZConst, TransformFlag.RotXConst, TransformFlag.RotYConst, TransformFlag.RotZConst, TransformFlag.PosXConst, TransformFlag.PosYConst, TransformFlag.PosZConst)
        for i in range(len(values)):
            if isinstance(values[i], float):
                struct += 'f'
                flags |= const_flags[i]
...
class CANMBoneBakedTransform(CANMBone):
    ...
    def refresh_struct(self):
        ...
        ignore_flags = (BakedTransformFlag.RotationIgnore, BakedTransformFlag.TranslationIgnore, BakedTransformFlag.ScaleIgnore)
        for i in range(len(values)):
            if values[i] is None:
                flags |= ignore_flags[i]
        self.flags = flags
